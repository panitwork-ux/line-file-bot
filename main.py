...
def upload_file(content, filename, folder_id):
    service = get_drive_service()
...
def search_files_recursive(folder_id, date_str=None, max_results=5):
    service = get_drive_service()
    results = []

    def search_in(fid):
        conditions = [f"'{fid}' in parents", "trashed=false",
        ...
        res = service.files().list(q=q, fields="files(id,name,webViewLink,createdTime)",
        ...
        subs = service.files().list(q=sub_q, fields="files(id)").execute()
...
def push_flex(to, file_name, sender_short, group_name, sub_folder_name, folder_url, file_url, upload_time, file_type):
    ...
    flex_content = {
        ...
        "footer": {
            ...
            "contents": [
                {"type": "button", "action": {"type": "uri", "label": "📂 เปิดดูโฟลเดอร์", "uri": folder_url},
                 "style": "primary", "color": "#1B5E20", "height": "sm"},
                {"type": "button", "action": {"type": "uri", "label": "📄 เปิดไฟล์นี้", "uri": file_url},
                 "style": "secondary", "height": "sm"}
            ]
        }
    }
    line_bot_api.push_message(to, FlexSendMessage(
        alt_text=f"✅ บันทึกไฟล์สำเร็จ! {file_name}", contents=flex_content))

def reply_search_results(reply_token, query, files):
    if not files:
        line_bot_api.reply_message(reply_token, TextSendMessage(text=f"🔍 ไม่พบไฟล์: {query}"))
        return
    bubbles = []
    for f in files[:5]:
        ...
        bubbles.append({
            ...
            "body": {
                ...
                "contents": [
                    ...
                    {"type": "text", "text": f["name"], "size": "xs", "wrap": True,
                     "weight": "bold", "margin": "sm", "align": "center"},
            ...
            "footer": {
                ...
                "contents": [{"type": "button",
                               "action": {"type": "uri", "label": "เปิดไฟล์", "uri": f["webViewLink"]},
                               "style": "primary", "color": "#1B5E20", "height": "sm"}]
            }
        })
...
def handle_text(event):
    ...
    if user_id in pending_files:
        ...
        if text == "➕ สร้างโฟลเดอร์ใหม่":
            ...
            line_bot_api.reply_message(reply_token, TextSendMessage(
                text="📝 พิมพ์ชื่อโฟลเดอร์ย่อยที่ต้องการสร้าง:"))
            return
        ...
        if pending.get("waitingForFolderName"):
            ...
            line_bot_api.reply_message(reply_token, TextSendMessage(
                text=f"⏳ กำลังสร้างโฟลเดอร์ \"{folder_name}\" และอัปโหลด..."))
        ...
        if text.startswith("📁 "):
            ...
            del pending_files[user_id]
            line_bot_api.reply_message(reply_token, TextSendMessage(
                text=f"⏳ กำลังอัปโหลดไปยัง 📁 {folder_name}..."))
            threading.Thread(target=do_upload, args=(push_to, folder_name, pending)).start()
            return

    if text == "ค้นหา":
        items = [
            QuickReplyButton(action=MessageAction(label="📅 ไฟล์วันนี้", text="ไฟล์วันนี้")),
    ...
    if text == "ไฟล์ของฉัน":
        ...
        group_name = get_group_name(group_id)
        group_folder_id = get_or_create_folder(group_name, ROOT_FOLDER_ID)
        files = search_files_recursive(group_folder_id)
        reply_search_results(reply_token, f"ไฟล์ของ {sender_name}", files)
        return
    ...
    if text in ["/help", "ช่วยเหลือ"]:
        line_bot_api.reply_message(reply_token, TextSendMessage(
            ...
            quick_reply=QuickReply(items=[
                QuickReplyButton(action=MessageAction(label="🔍 ค้นหา", text="ค้นหา"))
            ])))
...
def handle_file_message(event, file_type):
    ...
    if file_type == "file":
        file_name = getattr(msg, "file_name", None) or f"file_{ts}"
    elif file_type == "image":
        file_name = f"IMG_{ts}.jpg"
    elif file_type == "video":
        file_name = f"VID_{ts}.mp4"
    elif file_type == "audio":
        file_name = f"AUD_{ts}.m4a"
    else:
        file_name = f"FILE_{ts}"
